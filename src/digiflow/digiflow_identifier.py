...
def generate_urn_pz(urn):
    ...
    # map + reduce
    mapped_digits_str = functools.reduce(
        lambda c, p: c + p, [str(PZ_MAP[c]) for c in urn]
    )
...
class ProcessReport:
    ...
    def __init__(
        self, added: List[GranularContainer], existed: List[GranularContainer]
    ) -> None:
...
def enrich_urn_granular(
    xml_tree, urn_main, page_num=None, padd_left=4, do_sanitize=False, do_replace=False
):
    """
    ...
    # might happend with additional images (colorchecker, post scans, ...)
        ...
        try:
            add += insert_granular_urn(
                urn_type, cnts_total, urn_main, page_num, padd_left
            )
...
def _guess_urn_granular_from_existing(xml_tree, urn_main):
    ...
    if str(len_phys_div) == last_phys_div.attrib["ORDER"]:
        ...
        xml_tree, urn_type, rep = enrich_urn_granular(
            xml_tree, urn_main, page_num=page_num
        )
...
def _insert_page_fragment(phys_div, main_urn: str, padd: int):
    ...
    return GranularContainer(
        id=phys_div.attrib["ID"], type=URNType.GRANULAR_FRAGMENT_PAGE, urn=the_gran_urn
    )
...
def insert_granular_urn(urn_type, phys_conts, main_urn, page_num=None, padd_left=4):
    ...
    inserts = []
    els_missing = [
        e for e in phys_conts if not does_fit(e, pattern=DEFAULT_PATTERN_ULB)
    ]
    if urn_type == URNType.GRANULAR_PAGE:
        for el_misses in els_missing:
            ...
            cnts_with_gurn = [
                e
                for e in phys_conts
                if "CONTENTIDS" in e.attrib
                and re.match(r".*-p\d+-\d$", e.attrib["CONTENTIDS"])
            ]
            existing_ones = [
                e for e in cnts_with_gurn if e.attrib["CONTENTIDS"] == new_gurn
            ]
            if existing_ones:
            ...
            inserts.append(
                GranularContainer(
                    id=el_id, type=URNType.GRANULAR_FRAGMENT_PAGE, urn=new_gurn
                )
            )
    return inserts
...
def enrich_urn_kitodo2(
    process_path: "str|Path",
    collection="1",
    system_id=KITODO2_ID,
    exemplar="1",
    ...
    identifiers = read.xpath(
        './/goobi:metadata[@name="CatalogIDSource" and(not(@anchorId))]/text()'
    )
    ...
    # insert
    read.insert_into_prime_mods(
        "{http://meta.goobi.org/v1.5.1/}metadata", {"name": "ulb_urn"}, urn
    )
    read.write()
    _urns.append(urn)
    ...
    if os.path.exists(path_meta_anchor):
        ...
        urn_host = (
            f"urn:nbn:de:gbv:3:{collection}-{system_id}-{ppn_c_stage[0]}-{exemplar}"
        )
        pz_c_stage = generate_urn_pz(urn_host)
        urn_host += pz_c_stage
        _proc_host = MetsReader(path_meta_anchor)
        _proc_host.insert_into_prime_mods(
            "{http://meta.goobi.org/v1.5.1/}metadata", {"name": "ulb_urn"}, urn_host
        )
        _proc_host.write()
