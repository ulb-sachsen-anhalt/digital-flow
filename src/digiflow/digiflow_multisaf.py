...
def write_dc(element, qualifier, content, xmlfile):
    ...
    if qualifier != "":
        ...
        write_string = (
            '  <dcvalue element="' + str(element) + '">' + str(content) + "</dcvalue>\n"
        )
...
def find_files(file_list, name_pre, path):
    ...
    for dire in dirlist:
        ...
        file_list = find_files(
            file_list, name_pre + sdir + "_", path / pathlib.Path(dire)
        )
...
def create_saf(table_path, file_path_base, one_saf_input, finished_folder):
        ...
        try:
            ...
            if one_saf:
                ...
                if len(item_number_string) < number_of_items_string_length:
                    for _ in range(
                        number_of_items_string_length - len(item_number_string)
                    ):
                        item_number_string = "0" + item_number_string
                temp_folder = temp_folder_base / pathlib.Path(
                    "item_" + item_number_string
                )
            ...
            with open(
                temp_folder / "dublin_core.xml", "w", encoding="UTF-8"
            ) as xmlfile:
            ...
            with open(temp_folder / "collections", "w", encoding="UTF-8") as file:
                file.write(
                    str(
                        metadata_table.cell(
                            row=row_number, column=metadata_table_meta["collection"]
                        ).value
                    )
                )
            # Filehandling
            ...
            rights_group = metadata_table.cell(
                row=row_number, column=metadata_table_meta["dspacegroup"]
            ).value
            with open(temp_folder / "contents", "w", encoding="UTF-8") as file:
                if rights_group:
                    ...
                    for _, file_name in list_files_with_path:
                        file.write(
                            file_name + "\tpermissions: -r '" + rights_group + "'\n"
                        )
                else:
                    for _, file_name in list_files_with_path:
            ...
            with open(
                temp_folder / "metadata_local.xml", "w", encoding="UTF-8"
            ) as xmlfile:
                xmlfile.write('<dublin_core schema="local">\n')
                xmlfile.write(
                    '  <dcvalue element="accessrights" qualifier="dnb">free</dcvalue>\n'
                )
                if rights_group:
                    xmlfile.write(
                        '  <dcvalue element="openaccess" qualifier="">false</dcvalue>\n'
                    )
                else:
                    xmlfile.write(
                        '  <dcvalue element="openaccess" qualifier="">true</dcvalue>\n'
                    )
                xmlfile.write("</dublin_core>")
            # SAF-Creation
            ...
            if not one_saf:
                ...
                shutil.make_archive(
                    finished_folder / folder_name,
                    format="zip",
                    root_dir=temp_folder_base,
                )
    ...
    if one_saf:
        ...
        try:
            ...
            shutil.make_archive(
                finished_folder / project_name, format="zip", root_dir=temp_folder_base
            )
            shutil.rmtree(temp_folder_base)
        except Exception as errormessage:
            print("Error:")
    ...
    if not args.t:
        ...
        if not args.f:
            ...
            if not args.r:
                ...
                print(
                    "-r, Folder with finished SAFs, will be placed next to saf data folder:"
                    + str(args.r)
                )
