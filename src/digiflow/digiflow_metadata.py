...
# script constants
DEFAULT_FLAT_STRUCTS = ["monograph", "manuscript"]
...
def write_xml_file(
    xml_root, outfile, preamble='<?xml version="1.0" encoding="UTF-8"?>'
):
...
def _pretty_xml(xml_root, preamble='<?xml version="1.0" encoding="UTF-8"?>'):
    ...
    as_string = ET.tostring(
        ET.ElementTree(xml_root), pretty_print=True, encoding="UTF-8"
    )
...
def post_oai_extract_metsdata(xml_tree):
    ...
    if namespace == "http://www.openarchives.org/OAI/2.0/":
        ...
        check_error = xml_tree.xpath(
            ".//*[local-name() = 'error']", namespaces=dfc.XMLNS
        )
...
class XMLProcessor(abc.ABC):
    ...
    def __init__(self, input_xml, xmlns=None):
        ...
        if isinstance(input_xml, ET._Element):
            ...
            if not input_xml.is_file():
                raise DigiflowMetadataException(
                    f"{input_xml} considered to be a path but is not!"
                )
        ...
        if self.root is None:
            raise DigiflowMetadataException(
                f"Failed to process input type {type(input_xml)}"
            )
    ...
    def remove(self, tags):
        ...
        if self.root is None:
            raise DigiflowMetadataException(
                f"Failed to remove {tags} no XML root known!"
            )
        for tag in tags:
            removals = self.root.xpath(
                f'//*[local-name()="{tag}"]', namespaces=self.xmlns
            )
            for rem in removals:
    ...
    def findall(self, expression, element=None, get_all=True):
        ...
        if element is not None:
            ...
            if self.root is None:
                raise DigiflowMetadataException(
                    f"Failed to find {expression} no XML root known!"
                )
    ...
    def xpath(self, expression, element=None):
        ...
        if self.root is None:
            raise DigiflowMetadataException(
                f"Failed to execute {expression} no XML root known!"
            )
...
class MetsProcessor(XMLProcessor):
    ...
    def enrich_agent(self, agent_name, agent_note=None, **kwargs):
        ...
        agent_smith = ET.Element(
            "{http://www.loc.gov/METS/}agent", kwargs
        )  # pyright: ignore[reportCallIssue]
        the_name = ET.SubElement(
            agent_smith, "{http://www.loc.gov/METS/}name"
        )  # pyright: ignore[reportCallIssue]
        the_name.text = agent_name
        the_note = ET.SubElement(
            agent_smith, "{http://www.loc.gov/METS/}note"
        )  # pyright: ignore[reportCallIssue]
    ...
    def contains_group(self, group) -> bool:
        ...
        if isinstance(group, list):
            return any(
                self.root.findall(PATTERN_FILEGROUP_USE.format(g), dfc.XMLNS)
                for g in group
            )
        if isinstance(group, str):
            found_group = self.root.findall(
                PATTERN_FILEGROUP_USE.format(group), dfc.XMLNS
            )
    ...
    * Kitodo3 objects use UUIDs
    ...
    def _find_logical_root_id(self):
        ...
        * otherwise, guess prime by structure map
        ...
        if self.root is None:
            raise DigiflowMetadataException(
                f"Failed to find prime MODS ID in {self.path_xml}: No root known!"
            )
    ...
    def _determine_prime_dmd_id_by_logical_struct(self) -> str:
        if self.root is None:
            raise DigiflowMetadataException(
                f"Fail to determine prime dmd in {self.path_xml}: No root element found!"
            )
        ...
        if len(first_level) == 1:
            ...
            if the_type in DEFAULT_FLAT_STRUCTS:
                ...
                if not self.contains_group(CONTENT_FILE_GROUPS):
                    ...
                    xpr_root_link = (
                        f'.//mets:structLink/mets:smLink[@{XLINK_TO}="physroot"]'
                    )
                    ...
                    xpr_log = (
                        f'.//mets:structMap[@TYPE="LOGICAL"]//mets:div[@ID="{log_id}"]'
                    )
    ...
    def report(self):
        ...
        if self._report is None:
            ...
            self._report.logical_type, self._report.hierarchy = (
                self.inspect_logical_struct()
            )
    ...
    def insert_into_prime_mods(self, tag, attributes=None, text_content=None):
        """Insert element"""
        new_el = ET.SubElement(
            self.primary_dmd, tag, attrib=attributes
        )  # pyright: ignore[reportCallIssue]
        if text_content:
    ...
    def inspect_logical_struct_links(self):
        ...
        if self.root is None:
            raise DigiflowMetadataException(
                f"Failed to inspect logical struct links in {self.path_xml}: No root known!"
            )
        log_ids = self.root.xpath(
            f'.//mets:div[@DMDID="{self.dmd_id}"]/mets:div/@ID', namespaces=dfc.XMLNS
        )
        for log_id in log_ids:
            all_links = self.root.findall(
                f'.//mets:smLink[@{XLINK_FROM}="{log_id}"]', dfc.XMLNS
            )
            if not all_links:
                raise DigiflowMetadataException(
    ...
    def _get_dmd_identifier(self):
        ...
        """
        agent_notes = self.root.xpath(
            ".//mets:agent/mets:note/text()", namespaces=dfc.XMLNS
        )
        if agent_notes:
    ...
    def ulb_system_identifier(self) -> dict:
        ...
        * use METS-agent-information, if available
        ...
        repo_one = (
            ...
            else MARK_AGENT_LEGACY.split(":", maxsplit=1)[0]
        )
        # legacy migrated record found?
        legacy_marks = self.xpath(
            f'//mets:note[contains(text(), "{MARK_AGENT_LEGACY}")]/text()'
        )
        if len(legacy_marks) == 1:
            ...
            if ":" in legacy_id:
                ...
                legacy_ident = (
                    legacy_ident[2:] if legacy_ident.startswith("md") else legacy_ident
                )
                ident_dict[repo_one] = legacy_ident
            else:
                ident_dict[repo_one] = legacy_id
        legacy_vls_marks = self.xpath(
            f'//mets:note[contains(text(), "{MARK_AGENT_VLID}")]/text()'
        )
        ...
        if (
            repo_one
            and ("digital" in repo_one or "menadoc" in repo_one)
            and repo_one not in ident_dict
            and self.dmd_id is not None
        ):
        ...
        viewer_pres = self.root.xpath(
            './/dv:presentation[contains(./text(), "://opendata")]/text()',
            namespaces=dfc.XMLNS,
        )
    ...
    def set_files(self) -> None:
        """Gather resources/files for any present file group.

        Parses METS XML to extract all file references organized by fileGrp/@USE attribute.
        Creates a separate list of METSFile objects for each fileGroup (e.g., MAX, THUMBS,
        DEFAULT, FULLTEXT, DOWNLOAD).

        Important: The mets_files list is initialized inside the loop (line 681) to ensure
        each fileGroup gets its own independent list. Previous implementation had a bug where
        mets_files was initialized outside the loop, causing all fileGroups to share the same
        list reference and accumulate files from all groups.

        Stores results in self._files dictionary with structure:
            {"MAX": [METSFile, ...], "THUMBS": [METSFile, ...], ...}
        """

        assert self.root is not None
        the_groups = self.root.findall(".//mets:fileGrp", dfc.XMLNS)
        for the_group in the_groups:
            use_attr = the_group.attrib.get("USE", "OTHER")
            the_files = the_group.findall("mets:file", dfc.XMLNS)
            # Initialize fresh list for each fileGroup to avoid cross-contamination
            mets_files = []
            for a_file in the_files:
                assert (
                    "ID" in a_file.attrib
                ), f"invalid file without ID in {self.path_xml}"
                the_id = a_file.attrib["ID"]
                the_type = a_file.get("MIMETYPE", "application/octet-stream")
                the_loco = a_file.xpath("mets:FLocat", namespaces=dfc.XMLNS)
                assert (
                    len(the_loco) == 1
                ), f"invalid fLocat {the_loco} in {self.path_xml}"
                the_link = the_loco[0].attrib[XLINK_HREF]
                mets_files.append(
                    dfc.METSFile(
                        use=use_attr,
                        file_id=the_id,
            ...
            # Store the independent list for this fileGroup
            self._files[use_attr] = mets_files

    def get_files(self, use_group="MAX") -> typing.List[dfc.METSFile]:
        """Get files for a certain file group"""

        if use_group in self._files:
            return self._files[use_group]
        return []
...
class ModsReader(XMLProcessor):
    ...
    def report(self):
        ...
        if self._report is None:
            ...
            if the_shelfs:
                dmd_report.locations = (
                    the_shelfs[0] if len(the_shelfs) == 1 else the_shelfs
                )
    ...
    def get_type(self):
        ...
        goobi_picas = self.root.xpath(
            "mods:extension//*[@name='PICAType']/text()", namespaces=dfc.XMLNS
        )
    ...
    def get_identifiers(self):
        ...
        if self.root is None:
            raise DigiflowMetadataException(
                f"Failed to get identifiers in {self.path_xml}: No root known!"
            )
        idents = {}
        ...
        record_infos = self.root.findall(
            "mods:recordInfo/mods:recordIdentifier", dfc.XMLNS
        )
        for rec in record_infos:
            idents[rec.attrib["source"]] = rec.text
        goobi_srcs = self.root.xpath(
            "mods:extension//*[@name='CatalogIDSource' and not(@anchorId)]/text()",
            namespaces=dfc.XMLNS,
        )
        if len(goobi_srcs) > 0:
            idents["goobi:CatalogSourceID"] = goobi_srcs[0]
        goobi_parents = self.root.xpath(
            "mods:extension//*[@name='CatalogIDSource' and @anchorId]/text()",
            namespaces=dfc.XMLNS,
        )
        if len(goobi_parents) == 1:
            idents["goobi:anchorID"] = goobi_parents[0]
        goobi_vds = self.root.xpath(
            "mods:extension//*[starts-with(@name, 'VD')]", namespaces=dfc.XMLNS
        )
        if len(goobi_vds) > 0:
            for vd in goobi_vds:
                vd_key = vd.get("name")
        ...
        goobi_urns = self.root.xpath(
            "mods:extension//*[contains(@name, 'urn')]/text()", namespaces=dfc.XMLNS
        )
        if len(goobi_urns) == 1:
            key = "urn"
    ...
    def get_location_shelfs(self):
        ...
        langs = self.root.xpath(
            "mods:location//mods:shelfLocator/text()", namespaces=dfc.XMLNS
        )
        if len(langs) > 0:
            return langs
        goobi_doclangs = self.root.xpath(
            "mods:extension//*[@name='DocLanguage']", namespaces=dfc.XMLNS
        )
