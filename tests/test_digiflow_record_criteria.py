...
def test_record_set_some_other_state(oai_record_list):
    ...
    # act
    handler.states(criterias=[c_state], set_state="metadata_read", dry_run=False)
...
def test_record_list_time_range(oai_record_list):
    ...
    c_state = df_rcr.State("ocr_skip")
    c_range = df_rcr.Datetime(
        dt_from="2021-08-03_15:10:00", dt_to="2021-08-03_15:20:00"
    )
...
def test_record_handler_merge_plain(tmp_path):
    ...
    criterias = [df_rcr.Identifier("8853012"), df_rcr.State("upload_done")]
...
def test_record_handler_merge_only_done(tmp_path):
    ...
    # act
    merge_result = handler.merges(
        path_oai_list2, dry_run=False, other_require_state="upload_done"
    )
...
def test_record_handler_merge_ignore_failure(tmp_path):
    ...
    # act
    merge_result = handler.merges(
        path_oai_list2, dry_run=False, other_ignore_state="upload_done"
    )
    ...
    criterias = [df_rcr.Identifier("9510507"), df_rcr.State("ocr_fail")]
    assert handler.states(criterias=criterias) == 1
