...
<xsl:stylesheet xmlns:mets="http://www.loc.gov/METS/"
    ...
    <xsl:template match="//mets:mets">
        <svrl:fired-rule>
        <xsl:variable name="metsDayOrderedLabel">
        ...
        <xsl:choose>
            <xsl:when test="$isIssueOrAdditional and not($metsDayOrderedLabel = $modsDateIssued)">
                <svrl:failed-assert>
                    <xsl:attribute name="id">date_mets_to_mods</xsl:attribute>
                    <xsl:attribute name="role">fatal</xsl:attribute>
                    <svrl:text>Logisches Datum passt nicht zu in publication:dateIssued: <xsl:value-of select="$metsDayOrderedLabel"/> != <xsl:value-of select="$modsDateIssued"/></svrl:text>
                </svrl:failed-assert>
            </xsl:when>
            <xsl:otherwise>Datumsangaben passen <xsl:value-of select="$metsDayOrderedLabel"/> = <xsl:value-of select="$modsDateIssued"/></xsl:otherwise>
        </xsl:choose>
        </svrl:fired-rule>
    </xsl:template>
</xsl:stylesheet>
