...
def test_ddb_validate_opendata_44046_ignore_ident01(share_it_monography):
    ...
    # act
    whats_up: df_vddb.Report = df_vddb.Reporter(share_it_monography).get(
        ignore_ddb_rule_ids=["identifier_01"]
    )
...
def test_ddb_validate_kitodo2_legacy_monography_curated(tmp_path):
    ...
    result: df_vddb.Report = df_vddb.Reporter(mets_source).get(
        ignore_ddb_rule_ids=["fileSec_02"]
    )
...
def test_ddb_validate_kitodo2_vd18_153142537(tmp_path):
    ...
    # act
    reporter = df_vddb.Reporter(mets_source)
    result: df_vddb.Report = reporter.get(
        ignore_ddb_rule_ids=df_vddb.IGNORE_DDB_RULES_ULB, min_ddb_level="info"
    )
...
def test_report_about_kitodo2_morbio_1748529021_xsd(tmp_path):
    ...
    assert (
        result.xsd_errors[0][2]
        == "Element '{http://www.loc.gov/mods/v3}accessCondition', attribute 'href': The attribute 'href' is not allowed."
    )
